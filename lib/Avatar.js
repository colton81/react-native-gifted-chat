...
export function Avatar(props) {
    ...
    const renderAvatarComponent = useCallback(() => {
        ...
        if (currentMessage) {
            return (<GiftedAvatar avatarStyle={[styles[position].image, imageStyle?.[position]]} user={currentMessage.user} onPress={() => onPressAvatar?.(currentMessage.user)} onLongPress={() => onLongPressAvatar?.(currentMessage.user)}/>);
        }
        return null;
    }, [
        renderAvatar,
    ...
    if (!showAvatarForEveryMessage &&
        ...
        return (<View style={[styles[position].container, containerStyle?.[position]]}>
				<GiftedAvatar avatarStyle={[styles[position].image, imageStyle?.[position]]}/>
			</View>);
    }
    return (<View style={[
