...
function createMessagesWithHeaders(messages) {
    const result = [];
    let lastDate = null;
    messages.forEach((message) => {
        const messageDate = new Date(message.createdAt);
        const shouldAddHeader = !lastDate ||
...
function MessageContainer(props) {
    const { messages = [], user, isTyping = false, renderChatEmpty: renderChatEmptyProp, onLoadEarlier, inverted = false, loadEarlier = false, listViewProps, invertibleScrollViewProps, extraData = null, isScrollToBottomEnabled = false, scrollToBottomOffset = 200, alignTop = false, scrollToBottomStyle, infiniteScroll = false, isLoadingEarlier = false, renderTypingIndicator: renderTypingIndicatorProp, renderFooter: renderFooterProp, renderLoadEarlier: renderLoadEarlierProp, forwardRef, handleOnScroll: handleOnScrollProp, scrollToBottomComponent: scrollToBottomComponentProp, renderDay: renderDayProp, renderFloatingDayProp, } = props;
    ...
    const listData = useMemo(() => createMessagesWithHeaders(messages), [messages]);
    ...
    const changeScrollToBottomVisibility = useCallbackThrottled(50, (isVisible) => {
    ...
    const renderItem = useCallback(({ item, }) => {
        ...
        if (item.type === 'header') {
            ...
            return renderDayProp ? (<View>{renderDayProp(dayProps)}</View>) : (<Day {...dayProps}/>);
        }
        // Render message
    ...
    const renderCell = useCallback((props) => {
        ...
        const item = firstChild?.props?.currentMessage;
        // If no currentMessage, this is likely a header - just render without tracking
        if (!item) {
            return (<View {...props} onLayout={onLayoutProp}>
            {children}
          </View>);
        }
        /**
    ...
    return (<View style={[
      ...
      <AnimatedFlashList ref={forwardRef} data={listData} extraData={extraData} renderItem={renderItem} keyExtractor={keyExtractor} getItemType={getItemType} automaticallyAdjustContentInsets={true} contentInset={{ top: insets.top, bottom: insets.bottom }} contentInsetAdjustmentBehavior={"automatic"} {...invertibleScrollViewProps} ListEmptyComponent={renderChatEmpty} ListFooterComponent={ListFooterComponent} ListHeaderComponent={ListHeaderComponent} onScroll={scrollHandler} scrollEventThrottle={1} onStartReached={onStartReached} onStartReachedThreshold={0.05} {...listViewProps} onLayout={onLayoutList} CellRendererComponent={renderCell} maintainVisibleContentPosition={{
            autoscrollToBottomThreshold: 0.2,
            autoscrollToTopThreshold: 0.2,
            startRenderingFromBottom: true,
        }}/>
      <ScrollToBottomWrapper />
      <DayAnimated scrolledY={scrolledY} daysPositions={daysPositions} listHeight={listHeight} renderDay={renderFloatingDayProp} messages={messages} isLoadingEarlier={isLoadingEarlier}/>
    </View>);
}
