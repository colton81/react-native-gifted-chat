...
function MessageContainer(props) {
    ...
    const renderCell = (props) => {
        const handleOnLayout = (event) => {
            ...
            const newValue = {
                y,
                height,
                createdAt: new Date(props.children.props.data?.createdAt ?? 0).getTime(),
            };
        ...
        return (<AnimatedCellContainer {...props} onLayout={handleOnLayout}>{props.children}</AnimatedCellContainer>);
        // return (
        //   <View
        //     {...props}
    ...
    return (<View style={[
      ...
      <AnimatedFlatList extraData={extraData} ref={forwardRef} 
    //keyExtractor={keyExtractor}
    data={messages} renderItem={renderItem} inverted={inverted} automaticallyAdjustContentInsets={false} style={stylesCommon.fill} {...invertibleScrollViewProps} ListEmptyComponent={renderChatEmpty} ListFooterComponent={inverted ? ListHeaderComponent : ListFooterComponent} ListHeaderComponent={inverted ? ListFooterComponent : ListHeaderComponent} onScroll={scrollHandler} scrollEventThrottle={1} onEndReached={onEndReached} onEndReachedThreshold={0.1} {...listViewProps} onLayout={onLayoutList} CellRendererComponent={renderCell}/>
