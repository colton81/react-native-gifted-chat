...
export const useAbsoluteScrolledPositionToBottomOfDay = (listHeight, scrolledY, containerHeight, dayBottomMargin, dayTopOffset) => {
    const absoluteScrolledPositionToBottomOfDay = useDerivedValue(() => listHeight.value +
        scrolledY.value -
        containerHeight.value -
        dayBottomMargin -
        dayTopOffset, [listHeight, scrolledY, containerHeight, dayBottomMargin, dayTopOffset]);
...
export const useRelativeScrolledPositionToBottomOfDay = (listHeight, scrolledY, daysPositions, containerHeight, dayBottomMargin, dayTopOffset, createdAt) => {
    ...
    const currentDayPosition = useDerivedValue(() => {
        ...
        return daysPositionsArray.value.find((day, index) => {
            ...
            return (absoluteScrolledPositionToBottomOfDay.value < dayPosition ||
    ...
    const relativeScrolledPositionToBottomOfDay = useDerivedValue(() => {
        const scrolledBottomY = listHeight.value +
            scrolledY.value -
...
const DayWrapper = forwardRef((props, ref) => {
    ...
    ...rest } = props;
    return (<View ref={ref}>
			{renderDayProp ? (renderDayProp({ ...rest, createdAt: currentMessage.createdAt })) : (<Day {...rest} createdAt={currentMessage.createdAt}/>)}
		</View>);
});
const Item = (props) => {
    ...
    <View key={props.currentMessage._id.toString()}>
			...
			{renderMessageProp ? (renderMessageProp(rest)) : (<Message {...rest}/>)}
		</View>);
};
export default Item;
