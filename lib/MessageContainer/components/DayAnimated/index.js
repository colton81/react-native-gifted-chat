...
import Animated, { interpolate, runOnJS, useAnimatedReaction, useAnimatedStyle, useDerivedValue, useSharedValue, withTiming, } from "react-native-reanimated";
import { Day } from "../../../Day";
...
const DayAnimated = ({ scrolledY, daysPositions, listHeight, renderDay, messages, isLoadingEarlier, ...rest }) => {
    ...
    const messagesDates = useMemo(() => {
        ...
        for (let i = 1; i < messages.length; i++) {
            ...
            if (!isSameDay(previousMessage, message) ||
    ...
    const style = useAnimatedStyle(() => ({
        top: interpolate(relativeScrolledPositionToBottomOfDay.value, [
            -dayTopOffset,
            -0.0001,
            0,
    ...
    return (<Animated.View style={[stylesCommon.centerItems, styles.dayAnimated, style]} onLayout={handleLayout}>
			<Animated.View style={contentStyle} pointerEvents="none">
				{renderDay ? (renderDay({ ...rest, createdAt })) : (<Day {...rest} containerStyle={[
