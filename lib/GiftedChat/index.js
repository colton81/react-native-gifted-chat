...
function GiftedChat(props) {
    ...
    messageIdGenerator = () => (Math.random() + 1).toString(36).substring(7), user = {}, onSend, locale = "en", renderLoading, actionSheet = null, textInputProps, renderChatFooter = null, renderInputToolbar = null, bottomOffset = 0, focusOnInputWhenOpeningKeyboard = true, keyboardShouldPersistTaps = Platform.select({
        ...
        android: "always",
        default: "never",
    }), onInputTextChanged = null, maxInputLength = null, inverted = false, minComposerHeight = MIN_COMPOSER_HEIGHT, maxComposerHeight = MAX_COMPOSER_HEIGHT, isKeyboardInternallyHandled = true, } = props;
...
GiftedChatWrapper.append = (currentMessages = [], messages, inverted = false) => {
    if (!Array.isArray(messages)) {
        messages = [messages];
    }
    return inverted
...
GiftedChatWrapper.prepend = (currentMessages = [], messages, inverted = false) => {
    if (!Array.isArray(messages)) {
        messages = [messages];
    }
    return inverted
